...
import { showUndoToast, openModal, closeModal, openDeadlineModal } from './ui.js';
...
document.getElementById('commandSend').onclick = ()=>{
  ...
  input.value = '';
  updateCommandExamplesVisibility();
};
...
function updateCommandExamplesVisibility(){
  const input = document.getElementById('commandInput');
  const examples = document.getElementById('commandExamples');
  examples.style.display = (document.activeElement===input || input.value.trim()==='') && input.value.trim()==='' ? 'flex' : 'none';
}
document.getElementById('commandInput').addEventListener('input', updateCommandExamplesVisibility);
document.getElementById('commandInput').addEventListener('focus', updateCommandExamplesVisibility);
document.getElementById('commandInput').addEventListener('blur', ()=>{
  setTimeout(updateCommandExamplesVisibility, 150); // allow chip click to register first
});
document.querySelectorAll('#commandExamples .ex-chip').forEach(chip=>{
  chip.onclick = ()=>{
    const input = document.getElementById('commandInput');
...
/* Quick action shortcuts + assistant history modal */
...
document.querySelectorAll('.qa-chip').forEach(chip=>{
  chip.onclick = ()=>{
    const action = chip.dataset.qa;
    if(action==='plan-day'){ logAssistantTurn('user', 'Plan my day'); planMyDay(); }
    else if(action==='plan-week'){ logAssistantTurn('user', 'Plan my week'); planMyWeek(); }
    else if(action==='free-time'){ logAssistantTurn('user', 'Find free time'); handleQueryFree('today'); }
    else if(action==='optimize'){ logAssistantTurn('user', 'Optimize schedule'); runOptimizeSchedule(); }
    else if(action==='next'){ logAssistantTurn('user', "What's next?"); handleQueryNext(); }
    else if(action==='reschedule-missed'){ logAssistantTurn('user', 'Reschedule missed tasks'); rescheduleMissedTasks(); }
    else if(action==='deadline-task'){ openDeadlineModal(); }
  };
});
